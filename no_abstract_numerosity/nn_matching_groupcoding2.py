...
for sam in range(1, (num_dnum + 1)):
    ...
    # com_list = com_list[(com_list<=num_dnum) & (com_list>=1)]
    # com_list = np.unique(com_list)
    for com in range(1, (num_dnum + 1)):
        X_sam_row = data_train_sam[data_train_sam[:, 1] == sam]
        X_com_row = data_train_com[data_train_com[:, 1] == com]
        X_sam = X_sam_row[:, 3:]
        X_com = X_com_row[:, 3:]
        X_sam_row_list.append(X_sam_row)
        X_com_row_list.append(X_com_row)
...
X_sam_train = np.concatenate(tuple(X_sam_row_list), axis=0)
X_com_train = np.concatenate(tuple(X_com_row_list), axis=0)
...
for sam in range(1, (num_dnum + 1)):
    ...
    # com_list = np.unique(com_list)
    for com in range(1, (num_dnum + 1)):
        X_sam_row = data_val_sam[data_val_sam[:, 1] == sam]
        X_com_row = data_val_com[data_val_com[:, 1] == com]
        X_sam = X_sam_row[:, 3:]
        X_com = X_com_row[:, 3:]
        X_sam_row_list.append(X_sam_row)
        X_com_row_list.append(X_com_row)
...
X_sam_val = np.concatenate(tuple(X_sam_row_list), axis=0)
X_com_val = np.concatenate(tuple(X_com_row_list), axis=0)
...
for e in range(epochs):
    ...
    # 对图片进行推断，计算损失函数，反向传播优化权重，将损失求和
    
    # 这里还未读取data_numneu_train和labels_train
    ps = model(data_numneu_train)
    loss = criterion(ps, labels_train)
    loss.backward()
    optimizer.step()
...
# %%
# 保存模型需要代码，torch.save
save_path = '/nfs/s2/userhome/xuezhichao/workingdir/group_coding/match_classifier2_fc1_80.pth'
# save_path = 'E:/working/group_coding/match_classifier_fc1_80' + f'dset{dset}' + '.pth'
...
plt.legend()
picname = '/nfs/s2/userhome/xuezhichao/workingdir/group_coding/match_classifier2_fc1_80.png'
# picname = 'E:/working/group_coding/match_classifier_fc1_80' + f'dset{dset}' + '.png'
plt.savefig(picname)
plt.close()
